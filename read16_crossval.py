...
def _parse_args():
    ...
    parser.add_argument(
        ...
        default=5.5e-6,
        help=(
            "One-Cycle max LR (paper value is in supplementary); used when --huttner. "
            "The end LR is max_lr / 2.2e4 and the initial LR is fixed at 1e-9."
        ),
    )
    parser.add_argument(
        "--huttner_pct_start",
        ...
        default=0.1,
        help=(
            "One-Cycle warm-up fraction (pct_start). Paper optimized this; "
...
class TrOCRDataset(Dataset):
    ...
    def preprocess_image(self, img_np):
        ...
        # No contrast enhancement (closer to the paper's "no augmentation" setup)
        if len(img_np.shape) == 3:
...
if ARGS.huttner:
    ...
    # - One-Cycle LR with initial_lr=1e-9, end_lr=max_lr/2.2e4
    # - batch_size=8
    # - num_train_epochs=50 (READ16 full fine-tuning)
    # - no data augmentation (handled above)
    # - no label smoothing / early stopping / extra generation heuristics
...
if ARGS.huttner:
    # Keep generation simple (paper states beam search usage, but doesn't specify extra decoding heuristics)
